/* Formatted on 11/14/2020 10:13:59 AM (QP5 v5.287) */
  SELECT a.segment1,
         a.po_header_id,
         a.vendor_name,
         a.approved_date,
    ...
    FROM (SELECT pha.segment1,
                 ...
                 ish.ship_date,
                 ppett.name,
                 ( (allocation_amt) * NVL (icl.currency_conversion_rate, 1))
                    Estimated_Amount,
                 NULL Actual_Amount,
                 fa.asset_number,
                 fa.asset_id,
                 ...
                 fai.date_effective,
                 0 - NVL (fixed_assets_cost, 0) assets_amount
          ...
          UNION ALL
            SELECT po.segment1,
                   ...
                   fai.feeder_system_name,
                   fai.date_effective,
                   0 - SUM (NVL (fixed_assets_cost, 0)) assets_amount
              FROM xx_lc_details LC,
                   ap_invoices_all AI,
                   ap_suppliers sup,
                   ap_invoice_distributions_all ad,
            ...
            SELECT lc.po_number,
             ...
             WHERE     lc.lc_id = TO_NUMBER (ad.attribute1)
                   ...
                   AND lc.po_number NOT IN (SELECT segment1
                                              FROM po_headers_all)
                   --   and lc_number='147817020366'
                   AND (ai.org_id = :p_org_id OR :p_org_id IS NULL)
                   AND fa.asset_id = fai.asset_id(+)
                   AND fa.attribute15(+) = lc.lc_number
