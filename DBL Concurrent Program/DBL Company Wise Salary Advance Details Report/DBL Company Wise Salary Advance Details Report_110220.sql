/* Formatted on 2/11/2020 10:47:52 AM (QP5 v5.287) */
  SELECT employee_number,
         ...
         (SUM (NVL (opening_balance, 0))+SUM (NVL (debit_amount, 0))-SUM (NVL (credit_amount, 0))) closing_balance
    FROM (  SELECT employee_number,
                   employee_name,
                   designation,
                   (SUM (loan_taken)) - (NVL (SUM (loan_adjusted), 0))
                      opening_balance,
                   0 debit_amount,
                   0 credit_amount
              FROM (SELECT papf.employee_number,
                     ...
                     WHERE     1 = 1
                           ...
                           AND TO_DATE (peevf.effective_start_date) <
                                  TO_DATE ( :p_date_from)
                    ...
                    SELECT DISTINCT
                     ...
                     WHERE     papf.person_id = paaf.person_id
                           ...
                           AND TO_DATE (ppa.effective_date) <
                                  TO_DATE ( :p_date_from)
          ...
          GROUP BY employee_number, employee_name, designation
          UNION ALL
            SELECT employee_number,
                   employee_name,
                   designation,
