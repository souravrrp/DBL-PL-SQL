...
PIVOT
(
  SUM(opening_balance) open_bal,
  SUM(debit_amount) dr_amnt,
  SUM(credit_amount) cr_amt,
  SUM(closing_balance) cl_bal
  FOR PAYROLL_MONTH in (select (start_date,date_to) from dual)
);
...
,sum(opening_balance) open_bal
,sum(debit_amount) dr_amnt
,sum(credit_amount) cr_amnt
,(sum(opening_balance)+sum(debit_amount))-sum(credit_amount) cl_bal
FROM
(
SELECT employee_number,
         employee_name,
