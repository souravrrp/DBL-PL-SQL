/* Formatted on 6/15/2021 11:20:53 AM (QP5 v5.287) */
SELECT mph.manual_pi_number proforma_number,
       mph.status proforma_status,
       mph.manual_pi_date proforma_date,
       mph.customer_no customer_number,
       mph.customer_name,
       (SELECT    hl.address1
         ...
         WHERE     hca.cust_account_id = hcas.cust_account_id
          ...
          customer_address,
       mph.po_number customer_po_number,
       mph.style,
       mph.manual_bs_number bill_stat_number,
       mph.manual_bs_date bs_date,
       mph.merchandiser_name m_name,
       mph.payment_terms payment_term,
       mph.bank_name,
       mpl.article_name article_ticket,
       --mpl.item_code,
       mpl.item_description,
       mpl.quantity,
       mpl.unit_of_measure unit,
       (mpl.quantity / mpl.net_weight) net_weight,
       (mpl.quantity * mpl.gross_weight) gross_weight,
       mpl.unit_price price_per_unit,
       mpl.net_value
  --,mph.*
  --,mpl.*
