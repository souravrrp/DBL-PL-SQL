/* Formatted on 2/9/2022 4:42:31 PM (QP5 v5.374) */
  SELECT                                                        ---    ORG_ID,
         item_code,
         item_description,
         -- P_UOM,
         ...
         --             rcv_qty_rma_sqm,
         --         NVL (SUM (int_rcpt_qty_p), 0)
         --             int_rcpt_qty_p,
         --         NVL (SUM (int_rcpt_qty_s), 0)
         --             int_rcpt_qty_s,
         --         NVL (SUM (int_rcpt_qty_p), 0) * 0.092936803
         --             int_rcpt_qty_sqm,
         --         NVL (SUM (int_ship_qty_p), 0)
         --             int_ship_qty_p,
         --         NVL (SUM (int_ship_qty_s), 0)
         --             int_ship_qty_s,
         --         NVL (SUM (int_ship_qty_p), 0) * 0.092936803
         --             int_ship_qty_sqm,
         ...
         --             suom_write_off,
         --         NVL (SUM (write_off_qty_p), 0) * 0.092936803
         --             muom_write_off,
         ...
         (  NVL (SUM (open_bal_p), 0)
          ...
          + NVL (SUM (int_rcpt_qty_p), 0)
          + NVL (SUM (int_ship_qty_p), 0))    puom_cpbal,
         (  NVL (SUM (open_bal_s), 0)
          + NVL (SUM (rcv_qty_prod_s), 0)
          + NVL (SUM (rcv_qty_misc_s), 0)
          + NVL (SUM (rcv_qty_misc_ss), 0)
          ...
          + NVL (SUM (int_rcpt_qty_s), 0)
          + NVL (SUM (int_ship_qty_s), 0))    suom_cpbal,
           (  NVL (SUM (open_bal_p), 0)
            + NVL (SUM (rcv_qty_prod_p), 0)
            + NVL (SUM (rcv_qty_misc_p), 0)
            + NVL (SUM (rcv_qty_misc_pp), 0)
            ...
            + NVL (SUM (int_rcpt_qty_p), 0)
            + NVL (SUM (int_ship_qty_p), 0))
         * 0.092936803                        muom_cpbal
    FROM (                               ------------DBL CERAMICS LTD- IO(251)
          SELECT ood.organization_code,
                 ...
                 CASE SIGN (mmt.transaction_date - :p_date_from)
                     ...
                     ELSE
                         CASE mmt.transaction_type_id
                             WHEN 12 THEN mmt.primary_quantity
                             ELSE 0
                         END
                 END                             int_rcpt_qty_p,
                 CASE SIGN (mmt.transaction_date - :p_date_from)
                     ...
                     ELSE
                         CASE mmt.transaction_type_id
                             WHEN 12 THEN mmt.secondary_transaction_quantity
                             ELSE 0
                         END
                 END                             int_rcpt_qty_s,
                 CASE SIGN (mmt.transaction_date - :p_date_from)
                     ...
                     ELSE
                         CASE mmt.transaction_type_id
                             WHEN 21 THEN mmt.primary_quantity
                             ELSE 0
                         END
                 END                             int_ship_qty_p,
                 CASE SIGN (mmt.transaction_date - :p_date_from)
                     ...
                     ELSE
                         CASE mmt.transaction_type_id
                             WHEN 21 THEN mmt.secondary_transaction_quantity
                             ELSE 0
                         END
                 END                             int_ship_qty_s
            --                 CASE SIGN (
           ...
           WHERE     mmt.inventory_item_id = msi.inventory_item_id
                 ...
                 AND mmt.organization_id =
                     NVL ( :p_organization, mmt.organization_id)
                 AND mtln.transaction_date BETWEEN '01-JAN-2010'
                                               AND :p_date_to + .99999
          UNION ALL              ------------DBL CERAMICS BOGRA DEPOT- IO(256)
          SELECT ood.organization_code,
                 mmt.organization_id             org_id,
                 msi.inventory_item_id           item_id,
                 mic.segment2                    AS item_size,
...
GROUP BY item_code, item_description, grade_code;