/* Formatted on 4/25/2021 3:34:42 PM (QP5 v5.354) */
  SELECT                                                        ---    ORG_ID,
         item_code,
         item_description,
         -- P_UOM,
         ...
         (  NVL (SUM (open_bal_p), 0)
          ...
          + NVL (SUM (transfer_qty_p), 0))    puom_cpbal,
         (  NVL (SUM (open_bal_s), 0)
          ...
          + NVL (SUM (transfer_qty_s), 0))    suom_cpbal,
           (  NVL (SUM (open_bal_p), 0)
            + NVL (SUM (rcv_qty_prod_p), 0)
            + NVL (SUM (rcv_qty_misc_p), 0)
            + NVL (SUM (rcv_qty_misc_pp), 0)
    ...
    FROM (SELECT ood.organization_code,
          ...
          SELECT ood.organization_code,
                 ...
                 CASE SIGN (mtln.transaction_date - :p_date_from)
                     ...
                     ELSE
                         CASE mmt.transaction_type_id
                             WHEN 52 THEN mtln.primary_quantity
                             ELSE 0
                         END
                 END                             sales_qty_p,
                 CASE SIGN (mtln.transaction_date - :p_date_from)
                     ...
                     ELSE
                         CASE mmt.transaction_type_id
                             WHEN 52 THEN mtln.secondary_transaction_quantity
                             ELSE 0
                         END
                 END                             sales_qty_s,
                 CASE SIGN (mtln.transaction_date - :p_date_from)
           ...
           WHERE     mmt.inventory_item_id = msi.inventory_item_id
                 ...
                 --AND mlt.grade_code = 'A'
                 AND (logical_transaction = 2 OR logical_transaction IS NULL)
                 --AND mmt.transaction_type_id NOT IN (80, 120, 33, 26, 64)
                 AND mmt.subinventory_code IN ('CEM-REJECT',
                                               'CEM-STAG',
                                               'DBLCL-JKL2',
                                               'CEM-SAMPLE')
