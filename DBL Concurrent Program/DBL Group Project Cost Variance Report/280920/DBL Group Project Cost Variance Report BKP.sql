/* Formatted on 8/31/2020 12:20:18 PM (QP5 v5.287) */
  SELECT PROJECT_NAME,
         BUILDING_LEVEL_NAME,
         SUM (NVL (MATERIAL_BUDGET, 0)) MATERIAL_BUDGET,
         SUM (NVL (LABOR_BUDGET, 0)) LABOR_BUDGET,
         SUM (NVL (MATERIAL_CONSUMPTION, 0)) MATERIAL_CONSUMPTION,
         SUM (NVL (LABOR_CONSUMPTION, 0)) LABOR_CONSUMPTION,
         SUM (NVL (MATERIAL_BUDGET, 0)) - SUM (NVL (MATERIAL_CONSUMPTION, 0))
            MATERIAL_VARIANCE,
         SUM (NVL (LABOR_BUDGET, 0)) - SUM (NVL (LABOR_CONSUMPTION, 0))
            LABOR_VARIANCE,
         SUM (NVL (MATERIAL_PAYABLE, 0)) MATERIAL_PAYABLE,
         SUM (NVL (MATERIAL_PAYMENT, 0)) MATERIAL_PAYMENT
    FROM (  SELECT PROJECT_NAME,
          ...
          --                   xxdbl.com.GET_PROJECT_BUILDING_LEVEL (BUILDING_LEVEL_ID)
          UNION ALL
            SELECT xxdbl.COM.GET_PROJECT (AD.ATTRIBUTE1) PROJECT_NAME,
          ...
          UNION ALL
            SELECT xxdbl.com.get_project (rt.attribute10) PROJECT_NAME,
