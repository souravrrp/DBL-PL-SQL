/* Formatted on 4/6/2021 10:44:59 AM (QP5 v5.287) */
WITH AR_INVOICE
     AS (SELECT DISTINCT
                CT.ORG_ID,
                CT.LEGAL_ENTITY_ID,
                ...
                DECODE (CT.INVOICE_CURRENCY_CODE,'BDT',DIST.AMOUNT * 0,'USD',DIST.AMOUNT*1) ENT_AMOUNT,
          ...
          WHERE     DIST.ACCOUNT_CLASS = 'REC'
                ...
                AND TRUNC (DISTCM2.GL_DATE(+)) BETWEEN :P_DATE_FROM
                                                   AND :P_DATE_TO--AND CT.TRX_NUMBER IN ('420002214','421013941')
     ),
     BR_INVOICE
...
SELECT 1 SL,
  ...
  FROM (SELECT CUSTOMER_NUMBER,
               ...
               APPS.XX_AR_PKG.GET_CM_AMOUNT (AR.ORG_ID,
                  ...
                  CM_ACCTD_AMOUNT
          FROM AR_INVOICE AR
         WHERE     AR.LEGAL_ENTITY_ID = :P_LEGAL_ENTITY_ID
               AND ( :P_ORG_ID IS NULL OR AR.ORG_ID = :P_ORG_ID)
               AND (   :P_CUSTOMER_TYPE IS NULL
