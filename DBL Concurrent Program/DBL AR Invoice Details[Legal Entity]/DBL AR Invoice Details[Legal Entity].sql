/* Formatted on 22-Nov-18 09:51:49 (QP5 v5.136.908.31019) */
WITH AR_INVOICE
        AS (SELECT DISTINCT
                   CT.ORG_ID,
                   ...
                   CASE
                      WHEN CT.INVOICE_CURRENCY_CODE = 'BDT' THEN 0
                      ELSE DIST.AMOUNT
                   END
                      ENT_AMOUNT,
                   DIST.ACCTD_AMOUNT,
             ...
             WHERE     DIST.ACCOUNT_CLASS = 'REC'
                   ...
                   AND CT.TRX_NUMBER NOT LIKE 'IN5%'
                   ...
                   AND CT.ORG_ID = CLM.ORG_ID
                   AND CUST_SITE.ORG_ID = CLM.ORG_ID
                   ...
                   AND (:P_TRX_TYPE IS NULL
                        OR UPPER (RCTT.TYPE) = :P_TRX_TYPE)
                   AND (:P_SUB_UNIT IS NULL
                        OR UPPER (RCTT.ATTRIBUTE1) = UPPER (:P_SUB_UNIT))
                   AND (:P_TRX_TYPE_ID IS NULL
                        OR RCTT.CUST_TRX_TYPE_ID = :P_TRX_TYPE_ID)
  ...
  SELECT 0 SL,
         ...
         NVL (
            SUM(ABS(NVL (CM_ENT_AMOUNT_OA, 0)
         ...
         NVL (SUM (NVL (GREATEST (NVL (ADJUST_ENT_AMOUNT_OA, 0), 0), 0)), 0)
            ADJUST_ENT_AMOUNT,
         NVL (
            SUM(ABS(NVL (CM_ACCTD_AMOUNT_OA, 0)
         ...
         NVL (SUM (NVL (GREATEST (ADJUST_ACCTD_AMOUNT_OA, 0), 0)), 0)
            ADJUST_ACCTD_AMOUNT,
         SUM(CASE
                WHEN INVOICE_CURRENCY_CODE = 'BDT'
                THEN
                   0
                ELSE
                   (  ENT_AMOUNT
...
SELECT 1 SL,
  ...
  FROM (SELECT CUSTOMER_NUMBER,
               ...
               APPS.XX_AR_PKG.GET_CM_AMOUNT (AR.ORG_ID,
                                             ...
                                             :P_DATE_TO)
                  CM_ACCTD_AMOUNT
          FROM AR_INVOICE AR, XXDBL_COMPANY_LE_MAPPING_V CLM
         WHERE     AR.ORG_ID = CLM.ORG_ID
               AND CLM.LEDGER_ID = :P_LEDGER_ID
               AND (:P_ORG_ID IS NULL OR AR.ORG_ID = :P_ORG_ID)
               AND (:P_CUSTOMER_TYPE IS NULL
