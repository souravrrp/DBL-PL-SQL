...
AS
   ...
   IS
      ...
      l_date                DATE;
      --- Added by Mani 18-Jan-2021 --
      l_diff                NUMBER;
      l_max_percent         NUMBER;
      l_now_percent         NUMBER;
      l_fix_percent         NUMBER;
      l_max_rowid           VARCHAR2 (360);
      l_inv_id              VARCHAR2 (360);
...
--AND gbas.inventory_item_id = msi.inventory_item_id
      ...
      LOOP
         ...
         l_percent := ROUND (((r_main.final_cost / l_tot_amount) * 100), 7);
      ...
      COMMIT;
--- Added by Mani 18-Jan-2021 -- For testing --- Disabled
      /*FOR r_100_check IN
         (SELECT   allocation_id, SUM (fixed_percent) percent_calc
              FROM xxdbl_thd_expense_alloc_status
             WHERE run_id = l_run_id
               AND allocation_id NOT IN (
                                      SELECT DISTINCT allocation_id
      ...
      LOOP
         ...
         IF r_100_check.percent_calc > 100
         THEN
            l_diff := r_100_check.percent_calc - 100;
         END IF;

         BEGIN
            SELECT   MAX (fixed_percent)
                INTO l_max_percent
                FROM xxdbl_thd_expense_alloc_status
               WHERE run_id = l_run_id
                 AND allocation_id = r_100_check.allocation_id
                 AND allocation_id NOT IN (
                                      SELECT DISTINCT allocation_id
                                                 FROM xxdbl_thd_expense_alloc_status
                                                WHERE run_id = l_run_id
...
--            GROUP BY allocation_id
         ...
         THEN
            BEGIN
               SELECT ROWID
                ...
                WHERE run_id = l_run_id
                  AND allocation_id = r_100_check.allocation_id
                  AND fixed_percent = l_max_percent
                  AND allocation_id NOT IN (
                                      SELECT DISTINCT allocation_id
                                                 FROM xxdbl_thd_expense_alloc_status
                                                WHERE run_id = l_run_id
         ...
         THEN
            UPDATE xxdbl_thd_expense_alloc_status
               SET fixed_percent = fixed_percent - l_diff
             WHERE ROWID = l_max_rowid
               AND allocation_id = r_100_check.allocation_id;
         END IF;
      END LOOP;*/

      -- 01MAR2021 -- Mani and Sibu discussed this ---
            /*FOR r1 IN
               (SELECT DISTINCT allocation_id
                           FROM xxdbl_thd_expense_alloc_status
                          WHERE run_id = l_run_id
            ...
            LOOP
               ...
               writelog ('Allocation ID - ' ||  r1.allocation_id);

               SELECT SUM (ROUND(fixed_percent,7))
                 INTO l_tot_fixed_percent
                 FROM xxdbl_thd_expense_alloc_status
                WHERE run_id = l_run_id AND allocation_id = r1.allocation_id;

               writelog (' l_tot_fixed_percent - ' ||  l_tot_fixed_percent);

               l_remain_percent := ROUND((100 - l_tot_fixed_percent),7);

               writelog ('l_remain_percent - ' ||  l_remain_percent);

               IF l_remain_percent <> 0
               THEN
                  BEGIN
                  SELECT   ROUND(MAX (fixed_percent),7)
                      INTO l_max_percent
                      FROM xxdbl_thd_expense_alloc_status
                     WHERE run_id = l_run_id
      ...
      --            GROUP BY allocation_id
               ...
               writelog ('Max Percent - ' ||  l_max_percent);

               IF l_max_percent <> 0
               THEN
                  BEGIN
                     SELECT inventory_item_id,fixed_percent
                       INTO l_inv_id,l_fix_percent
                       FROM xxdbl_thd_expense_alloc_status
                      WHERE run_id = l_run_id
                        AND allocation_id = r1.allocation_id
                        AND ROUND(fixed_percent,7) =ROUND(l_max_percent,7)
                        AND allocation_id NOT IN (
                                            SELECT DISTINCT allocation_id
                                                       FROM xxdbl_thd_expense_alloc_status
                                                      WHERE run_id = l_run_id
               ...
               writelog ('Max Percent Inv ID - ' ||  l_inv_id);
               writelog ('Fixed Percent ID Before change - ' ||  l_fix_percent);

               l_now_percent :=  ROUND((l_fix_percent + l_remain_percent),7);

                  UPDATE xxdbl_thd_expense_alloc_status
                     SET fixed_percent = ROUND((fixed_percent + l_remain_percent),7)
                   WHERE inventory_item_id = l_inv_id
                     AND allocation_id = r1.allocation_id
                     AND run_id = l_run_id;

               writelog ('Fixed Percent ID After change - ' ||  l_now_percent);
...
-- Added by  Manas on 11-Nov-2020 Ends
      ...
      COMMIT;
      writelog ('--------------- Allocation Max Check Logic --------');
      lp_allocation_id := NULL;

      BEGIN
         SELECT DISTINCT alloc_id
                    INTO lp_allocation_id
                    FROM gmf.gl_aloc_mst
                   WHERE alloc_code = p_allocation_code;
      EXCEPTION
         WHEN OTHERS
         THEN
            lp_allocation_id := NULL;
      ...
      FOR r_gmf IN (SELECT   alloc_id, SUM (fixed_percent)
                                                          total_fixed_percent
                        FROM gmf.gl_aloc_bas gm
                       WHERE gm.alloc_id = NVL (lp_allocation_id, gm.alloc_id)
                         AND EXISTS (
                                SELECT 1
                                  FROM xxdbl_thd_expense_alloc_status xx
                                 WHERE xx.allocation_id = gm.alloc_id
                                   AND status <> 'E')
                    GROUP BY alloc_id)
      LOOP
         writelog ('Allocation ID - ' || r_gmf.alloc_id);
         l_remain_percent := 0;
         l_tot_fixed_percent := r_gmf.total_fixed_percent;
         l_inventory_item_id := NULL;
         l_diff := 0;
         l_max_percent := 0;
         l_now_percent := 0;
         ...
         l_max_rowid := 'X';
         l_inv_id := 'X';
         writelog ('Total Fixed Percent - ' || l_tot_fixed_percent);
         l_remain_percent := ROUND ((100 - l_tot_fixed_percent), 7);
         writelog ('Difference Percent - ' || l_remain_percent);
         ...
         THEN
            BEGIN
               SELECT ROUND (MAX (fixed_percent), 7)
                ...
                WHERE alloc_id = r_gmf.alloc_id
                                               --            GROUP BY allocation_id
               ;

               writelog (   'Inventory Max Percent in this Allocation - '
                         || l_max_percent
                        );
               ...
               THEN
                  BEGIN
                     SELECT inventory_item_id, fixed_percent
                       ...
                       FROM gmf.gl_aloc_bas
                      WHERE alloc_id = r_gmf.alloc_id
                        AND ROUND (fixed_percent, 7) =
                                                      ROUND (l_max_percent, 7)
                        AND ROWNUM = 1;
                  EXCEPTION
                     WHEN OTHERS
                     THEN
         ...
         writelog ('Max Percent Inventory ID - ' || l_inv_id);
         writelog (   'Fixed Percent for this Inventory Before change - '
                   || l_fix_percent
                  );
         l_now_percent := ROUND ((l_fix_percent + l_remain_percent), 7);
         ...
         UPDATE gmf.gl_aloc_bas
            SET fixed_percent = ROUND ((fixed_percent + l_remain_percent), 7)
          WHERE inventory_item_id = l_inv_id AND alloc_id = r_gmf.alloc_id;

         writelog (   'Fixed Percent for this Inventory After change - '
                   || l_now_percent
                  );

         UPDATE xxdbl_thd_expense_alloc_status
            SET fixed_percent = l_now_percent
          WHERE inventory_item_id = l_inv_id
            AND allocation_id = r_gmf.alloc_id
            AND run_id = l_run_id;
      END LOOP;
