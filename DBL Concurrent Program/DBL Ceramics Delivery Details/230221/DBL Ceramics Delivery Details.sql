/* Formatted on 5/9/2021 10:29:16 AM (QP5 v5.354) */
  SELECT CUSTOMER_NUMBER,
    ...
    FROM (SELECT AC.CUSTOMER_NUMBER,
                 ...
                 TH.VEHICLE_NO
                     VEHICLE_NUMBER,
                 TH.DELIVERY_MODE_CODE
                     VEHICLE_TYPE,
                 TH.DRIVER_NAME
                     DRIVER_NAME,
                 TH.TRANSPORTER_NO
                     DRIVER_CONTACT_NO,
                 (SELECT ARC.SHIP_TO_ORG_ADDRESS
                    FROM APPS.XX_AR_CUSTOMER_SITE_V ARC
                   WHERE     ARC.SHIP_TO_ORG_ID = OLA.SHIP_TO_ORG_ID
                         AND ARC.ORG_ID = OLA.ORG_ID
                         AND ARC.CUSTOMER_ID = OHA.SOLD_TO_ORG_ID)
                     SHIP_TO_ADDRESS,
                 (SELECT CT.TRX_NUMBER
                   ...
                   WHERE     1 = 1
                         ...
                         AND CL.INTERFACE_LINE_ATTRIBUTE6 = OLA.LINE_ID
                         AND CL.DESCRIPTION = MSI.DESCRIPTION
                         AND CL.INVENTORY_ITEM_ID = OLA.INVENTORY_ITEM_ID)
                     INVOICE_NUMBER
