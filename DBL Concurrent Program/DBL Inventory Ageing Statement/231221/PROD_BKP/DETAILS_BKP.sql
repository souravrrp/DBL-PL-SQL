/* Formatted on 12/12/2021 11:02:46 AM (QP5 v5.374) */
  SELECT ORGANIZATION_CODE,
         ...
         LAST_ISSUE_DATE
             LAST_ISSUE_DATE,
         TO_CHAR (TO_DATE (EXPIRY_DATE, 'YYYY/MM/DD'), 'DD-MON-YYYY')
             AS EXPIRY_DATE,
         ROUND (
             CEIL (
                   (  (APPS.XX_COM_PKG.GET_SECCOND_FROM_TIME_DIFF (
                           SYSDATE,
    ...
    FROM (  SELECT OOD.ORGANIZATION_CODE,
                   ...
                   LOT.EXPIRY_DATE,
                   (CASE
                        WHEN MIC.SEGMENT2 = 'FINISH GOODS'
                        THEN
                   ...
                   (CASE
                        ...
                        THEN
                            (SELECT NVL (MAX (TRUNC (A.TRANSACTION_DATE)),
                                         '30-SEP-2017')
                               FROM INV.MTL_MATERIAL_TRANSACTIONS A
                              WHERE     TRANSACTION_TYPE_ID IN (35, 31, 32)
                                    AND A.INVENTORY_ITEM_ID =
                                        MMT.INVENTORY_ITEM_ID
                                    AND A.ORGANIZATION_ID = MMT.ORGANIZATION_ID
                                    AND A.SUBINVENTORY_CODE =
