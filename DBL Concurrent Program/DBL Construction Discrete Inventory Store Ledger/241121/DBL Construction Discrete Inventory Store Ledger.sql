/* Formatted on 11/24/2021 2:40:16 PM (QP5 v5.365) */
WITH
    ITM
    AS
        ...
        (  SELECT ORGANIZATION_ID,
             ...
             FROM (       --------------------------MOVE_ORD_ISSUE------------
                   ...
                   GROUP BY MMT.ORGANIZATION_ID,
                            MMT.INVENTORY_ITEM_ID,
                            MMT.ACTUAL_COST
                   UNION ALL --------------MISCELLANEOUS_RECEIPT,MOVE_ORDER_RETURN----------------
                     SELECT MMT.ORGANIZATION_ID,
                            MMT.INVENTORY_ITEM_ID,
                      ...
                      WHERE     (   :P_ORG_ID IS NULL
                            ...
                            AND TRUNC (MMT.TRANSACTION_DATE) BETWEEN :P_DATE_FR
                                                                 AND :P_DATE_TO
                            AND MMT.TRANSACTION_TYPE_ID IN (42, 121)
                            AND TRANSACTION_ACTION_ID != 24
                            AND SIGN (PRIMARY_QUANTITY) = 1
                   GROUP BY MMT.ORGANIZATION_ID,
                            MMT.INVENTORY_ITEM_ID,
  ...
  SELECT MMT.ORGANIZATION_ID,
         ...
         SUM (MMT.RTN_ISU_VAL)
             RTN_ISU_VAL,
         SUM (MMT.INT_ORG_RCV_QTY)
             INT_ORG_RCV_QTY,
         SUM (MMT.INT_ORG_RCV_VAL)
             INT_ORG_RCV_VAL,
         SUM (MMT.SUB_INV_RCV_QTY)
             SUB_INV_RCV_QTY,
         SUM (MMT.SUB_INV_RCV_VAL)
             SUB_INV_RCV_VAL,
         SUM (MMT.RCV_QTY)
             RCV_QTY,
         SUM (MMT.RCV_VAL)
             RCV_VAL,
         SUM (MMT.INT_ORG_ISU_QTY)
             INT_ORG_ISU_QTY,
         SUM (MMT.INT_ORG_ISU_VAL)
             INT_ORG_ISU_VAL,
         SUM (MMT.SUB_INV_ISU_QTY)
             SUB_INV_ISU_QTY,
         SUM (MMT.SUB_INV_ISU_VAL)
             SUB_INV_ISU_VAL,
         SUM (MMT.ISU_QTY)
             ISU_QTY,
         SUM (MMT.ISU_VAL)
             ISU_VAL,
         (SUM (MMT.RCV_QTY) + SUM (MMT.ISU_QTY))
             NET_RCV_QTY,
         (SUM (MMT.RCV_VAL) + SUM (MMT.ISU_VAL))
             NET_RCV_VAL,
         SUM (MMT.MO_ISU_QTY)
             MO_ISU_QTY,
         SUM (MMT.MO_ISU_VAL)
    ...
    FROM (SELECT ITM.ORGANIZATION_ID,
          ...
          SELECT ITM.ORGANIZATION_ID,
           ...
           WHERE     1 = 1
                 ...
                 AND ITM.INVENTORY_ITEM_ID = CON.INVENTORY_ITEM_ID) MMT
