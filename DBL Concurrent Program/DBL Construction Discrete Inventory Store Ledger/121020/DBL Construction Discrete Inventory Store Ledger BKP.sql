/* Formatted on 10/12/2020 12:29:27 PM (QP5 v5.354) */
WITH
    MAINS
    AS
             ...
             FROM (       --------------------------PO_RECEIPT----------------
                     SELECT MMT.ORGANIZATION_ID,
                            ...
                            SUM (MMT.PRIMARY_QUANTITY)
                                PO_RCV_QTY,
                            SUM (MMT.PRIMARY_QUANTITY) * AVG (MMT.ACTUAL_COST)
                                PO_RCV_VAL,
                            TO_NUMBER (0)
                                RTN_ISU_QTY,
                            TO_NUMBER (0)
                   ...
                   UNION ALL --------------------------SUB_INV_TRNS------------
                     SELECT MMT.ORGANIZATION_ID,
                            ...
                            SUM (MMT.PRIMARY_QUANTITY) * AVG (MMT.ACTUAL_COST)    SUB_INV_RCV_VAL
                       FROM MTL_MATERIAL_TRANSACTIONS MMT
                      WHERE     (   :P_ORG_ID IS NULL
                                 OR MMT.ORGANIZATION_ID = :P_ORG_ID)
                            AND (   :P_SUB_INVENTORY IS NULL
             ...
             FROM (        ---------INTRANSIT_SHIPMENT+INTER_ORG_TRNS---------
                   ...
                   UNION ALL --------------------------SUB_INV_TRNS------------
                     SELECT MMT.ORGANIZATION_ID,
                            ...
                            SUM (MMT.PRIMARY_QUANTITY) * AVG (MMT.ACTUAL_COST)    SUB_INV_ISU_VAL
                       FROM MTL_MATERIAL_TRANSACTIONS MMT
                      WHERE     (   :P_ORG_ID IS NULL
                                 OR MMT.ORGANIZATION_ID = :P_ORG_ID)
                            AND (   :P_SUB_INVENTORY IS NULL
             ...
             FROM (       --------------------------MOVE_ORD_ISSUE------------
                   ...
                   UNION ALL --------------MISCELLANEOUS_RECEIPT----------------
                     SELECT MMT.ORGANIZATION_ID,
                            ...
                            SUM (MMT.PRIMARY_QUANTITY) * AVG (MMT.ACTUAL_COST)    MSC_RCV_VAL
                       FROM MTL_MATERIAL_TRANSACTIONS MMT
                      WHERE     (   :P_ORG_ID IS NULL
                                 OR MMT.ORGANIZATION_ID = :P_ORG_ID)
                            AND (   :P_SUB_INVENTORY IS NULL
  ...
  SELECT M.ORGANIZATION_ID,
         ...
         I.ISU_QTY,
         I.ISU_VAL,
         (R.RCV_QTY + I.ISU_QTY)                                         NET_RCV_QTY,
         (R.RCV_VAL + I.ISU_VAL)                                         NET_RCV_VAL,
         N.MO_ISU_QTY,
         N.MO_ISU_VAL,
         N.MSC_RCV_QTY,
         N.MSC_RCV_VAL,
         N.CON_QTY,
         N.CON_VAL,
         (R.RCV_QTY + I.ISU_QTY + N.CON_QTY)                             CLOSING_QTY,
         (R.RCV_VAL + I.ISU_VAL + N.CON_VAL)                             CLOSING_VAL,
         C.CLS_QTY,
         C.CLS_VAL,
