/* Formatted on 3/21/2021 10:38:30 AM (QP5 v5.287) */
CREATE OR REPLACE PACKAGE BODY XXDBL_TEST_XML_GEN_PKG
IS
   --Global variable
   ...
   PROCEDURE P_LOG (p_message IN VARCHAR2)
   IS
   BEGIN
   ...
   ----=================================================================================================================
   PROCEDURE P_OUTPUT (p_message IN VARCHAR2)
   IS
   BEGIN
      ...
      IS
         SELECT papf.first_name,
          ...
          WHERE     papf.person_id = paaf.person_id
                ...
                AND TRUNC (cv_effective_date) BETWEEN paaf.effective_start_date
                                                  AND paaf.effective_end_date
                AND TRUNC (cv_effective_date) BETWEEN papf.effective_start_date
                                                  AND papf.effective_end_date
   ...
   BEGIN
      ...
      FOR cur_emp_dtls_rec IN cur_emp_dtls (l_effective_date, p_person_id)
      LOOP
         g_step_id := 3;
         l_xml_tag := '<EmpDtls>';
         P_OUTPUT (l_xml_tag);

         FOR cur_mgr_dtls_rec IN cur_mgr_dtls
         LOOP
            l_xml_tag :=
                  '<FromEmailAddress>'
               || cur_mgr_dtls_rec.email_address
               || '</FromEmailAddress>';
            P_OUTPUT (l_xml_tag);
            g_step_id := 8.1;
            l_xml_tag :=
                  '<ToEmailAddress>'
               || cur_emp_dtls_rec.email_address
               || '</ToEmailAddress>';
            P_OUTPUT (l_xml_tag);
            g_step_id := 8.2;
            l_xml_tag :=
               '<CcEmailAddress>' || 'hr@xyz.com' || '</CcEmailAddress>';
            P_OUTPUT (l_xml_tag);
            g_step_id := 9;
            l_xml_tag :=
                  '<ReplyToAddress>'
            ...
            l_xml_tag :=
                  '<EmailSubject>'
               || 'Grade Letter for '
               || cur_emp_dtls_rec.full_name
               || '(Employee number '
               || cur_emp_dtls_rec.employee_number
               || ')'
               || '</EmailSubject>';
            P_OUTPUT (l_xml_tag);
            g_step_id := 11;
         ...
         l_xml_tag :=
               '<EmpFirstName>'
            || cur_emp_dtls_rec.first_name
            || '</EmpFirstName>';
         P_OUTPUT (l_xml_tag);
         --Employee Last Name
         g_step_id := 13;
         l_xml_tag :=
            '<EmpLastName>' || cur_emp_dtls_rec.last_name || '</EmpLastName>';
         P_OUTPUT (l_xml_tag);
