/* Formatted on 9/28/2020 9:40:24 AM (QP5 v5.354) */
  SELECT                                                        ---    ORG_ID,
         item_code,
         item_description,
         -- P_UOM,
         ...
         * 0.092936803                                    rcvq_misc_sqm,
         ...
         (  NVL (SUM (open_bal_p), 0)
          ...
          + NVL (SUM (broken_rec_qty_p), 0))              puom_cpbal,
         (  NVL (SUM (open_bal_s), 0)
          ...
          + NVL (SUM (broken_rec_qty_s), 0))              suom_cpbal,
           (  NVL (SUM (open_bal_p), 0)
            + NVL (SUM (rcv_qty_prod_p), 0)
            + NVL (SUM (rcv_qty_misc_p), 0)
            + NVL (SUM (rcv_qty_misc_pp), 0)
...
SELECT *
 ...
 WHERE 1 = 1                                           --AND batch_no = '3406'
             AND organization_id = 152 AND routing_id = 41;

SELECT *
  FROM apps.gmd_routings
 WHERE 1 = 1 AND routing_id = 41;

SELECT *
  FROM inv.mtl_material_transactions mmt
 WHERE     1 = 1
       AND TO_CHAR (transaction_date, 'DD-MON-RRRR') = '08-AUG-2020'
