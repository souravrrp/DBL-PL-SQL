/* Formatted on 5/6/2021 10:11:12 AM (QP5 v5.354) */
  SELECT item_code,
         item_description,
         item_id,
         item_type,
         ...
         * 0.092936803
         ...
         NVL (SUM (pick_qty_s), 0)
             ctn_pick,
         NVL (SUM (pick_qty_p), 0) * 0.092936803
             sqm_pick,
         ...
         NVL (SUM (wip_issue_p), 0) * 0.092936803
             wip_issue_sqm,
         NVL (SUM (accident_qty_p), 0)
             sft_accident,
         NVL (SUM (accident_qty_s), 0)
             ctn_accident,
         NVL (SUM (accident_qty_p), 0) * 0.092936803
             sqm_accident,
         ...
         * 0.092936803
         ...
         NVL (SUM (sample_qty_p), 0) * 0.092936803
             sqm_sample,
         NVL ((SUM (broken_qty_p) + SUM (accident_qty_p)), 0)
             sft_broken,
         NVL ((SUM (broken_qty_s) + SUM (accident_qty_s)), 0)
             ctn_broken,
         NVL ((SUM (broken_qty_p) + SUM (accident_qty_p)), 0) * 0.092936803
             sqm_broken,
    ...
    FROM (SELECT ood.organization_code,
           ...
           WHERE     mmt.inventory_item_id = msi.inventory_item_id
                 ...
                 AND mlt.lot_number = NVL ( :p_lot_number, mlt.lot_number)
