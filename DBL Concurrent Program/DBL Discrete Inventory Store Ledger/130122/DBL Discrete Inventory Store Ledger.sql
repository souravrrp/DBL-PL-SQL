/* Formatted on 1/13/2022 12:41:19 PM (QP5 v5.374) */
WITH
    MAINS
    AS
        (SELECT MSIK.ORGANIZATION_ID,
          ...
          WHERE     MSIK.ORGANIZATION_ID = MIC.ORGANIZATION_ID
                ...
                --AND (   :P_LEGAL_ENTITY IS NULL OR OOD.LEGAL_ENTITY = :P_LEGAL_ENTITY)
                AND OOD.LEGAL_ENTITY =
                    NVL ( :P_LEGAL_ENTITY, OOD.LEGAL_ENTITY)
                -- AND INVENTORY_ITEM_STATUS_CODE<>'Inactive'
                --AND ( :P_ORG_ID IS NULL OR MSIK.ORGANIZATION_ID = :P_ORG_ID)
                AND MSIK.ORGANIZATION_ID =
                    NVL ( :P_ORG_ID, MSIK.ORGANIZATION_ID)
                ...
                AND MSIK.INVENTORY_ITEM_ID =
                    NVL ( :P_ITEM_ID, MSIK.INVENTORY_ITEM_ID)
                AND MIC.SEGMENT2 = NVL ( :P_MJR_CAT, MIC.SEGMENT2)
                AND MIC.SEGMENT3 = NVL ( :P_MNR_CAT, MIC.SEGMENT3)),
    TRANS
    AS
             ...
             FROM (  SELECT MMT.ORGANIZATION_ID,
                      ...
                      WHERE     1 = 1
                            ...
                            AND MMT.INVENTORY_ITEM_ID =
                                NVL ( :P_ITEM_ID, MMT.INVENTORY_ITEM_ID)
