/* Formatted on 31/Jul/21 17:42:48 (QP5 v5.354) */
SELECT ac.customer_number,
       ac.customer_name,
       ac.customer_category_code             cust_category,
       acd.customer_name                     corosponding_dealer,
       ola.attribute1                        dealer_agreed_price,
       (CASE
       ...
       ABS (NVL (pcm.adjusted_amount, 0))    cssm,
       CASE
           WHEN transaction_type_id IN ('1010', '1008', '1030')
           THEN
       ...
       CASE
           WHEN transaction_type_id IN ('1010', '1008', '1030')
           THEN
                 (ola.shipped_quantity * (ABS (NVL (pcm.adjusted_amount, 0))))
               * (-1)
           ELSE
               ola.shipped_quantity * (ABS (NVL (pcm.adjusted_amount, 0)))
       END                                   csm_value,
       ((CASE
             WHEN transaction_type_id IN ('1010', '1008', '1030')
             THEN
                 (ola.shipped_quantity * ola.unit_list_price) * (-1)
             ELSE
                 ola.shipped_quantity * ola.unit_list_price
         END) --- (ola.shipped_quantity * (ABS (NVL (pcm.adjusted_amount, 0))))
             )                               total_value,
  ...
  FROM oe_order_headers_all          oha,
       ...
       (SELECT header_id, line_id, adjusted_amount
          FROM oe_price_adjustments_v pav
         WHERE adjustment_name = 'SO Header Adhoc Discount') pav,
       (SELECT header_id, line_id, adjusted_amount
          FROM oe_price_adjustments_v pav
         WHERE adjustment_name = 'CSSM') pcm
 WHERE     oha.header_id = ola.header_id
       ...
       AND oha.header_id = pcm.header_id(+)
       AND ola.line_id = pcm.line_id(+)
       AND ctl.description NOT LIKE '%Discount%'
       AND ctl.description NOT LIKE 'CSSM%'
...
SELECT ac.customer_number,
       ...
       * ola.ordered_quantity
       ...
       END
           freight_value,
         (  ola.unit_list_price
          - (ola.unit_selling_price - ABS (NVL (pcm.adjusted_amount, 0))))
       * ola.ordered_quantity
           total_discount,
       ola.shipped_quantity * ola.unit_selling_price
           invoice_amount,
