/* Formatted on 3/1/2021 5:11:28 PM (QP5 v5.287) */
  SELECT PRICE_LIST_NAME,
         ...
         LINE_DISCOUNT,
         ITEM_GRADE
    FROM (SELECT lhv.name price_list_name,
           ...
           WHERE     lhv.list_header_id = llv.list_header_id
                 ...
                 AND qpal.pricing_attr_value_from =
                        qpall.pricing_attr_value_from(+)
                 ...
                 AND qpal.pricing_attr_value_from =
                        qpall2.pricing_attr_value_from(+)
                 AND qsl2.name = 'CSSM'
          UNION ALL
          SELECT lhv.name price_list_name,
                 ...
                 llv.operand list_price,
                 0 cssm,
                 llv.operand total_price,
                 NVL (qq.operand, 0) line_discount,
                 qpal.pricing_attr_value_from item_grade
            FROM qp_list_headers lhv,
                 qp_list_lines_v llv,
                 qp_price_formulas qpf,
                 ...
                 mtl_units_of_measure_vl uom,
                 (SELECT qms.operand,
                         qms.product_attr_value,
                         qpall.pricing_attr_value_from
                    FROM qp_modifier_summary_v qms,
                         qp_secu_list_headers_vl qsl,
                         qp_pricing_attributes qpall
                   WHERE     qsl.name = 'Ceramic Discount - Sqft wise'
                         AND qms.list_line_id = qpall.list_line_id(+)
                         AND qms.list_header_id = qsl.list_header_id(+)
           ...
           WHERE     lhv.list_header_id = llv.list_header_id
                 ...
                 --AND msi.segment1 = 'GL3060-006GN'
                 AND TRUNC (SYSDATE) BETWEEN TRUNC (llv.start_date_active)
                                         AND TRUNC (llv.end_date_active)
                 AND msi.segment1 = qq.product_attr_value(+)
                 AND qpal.pricing_attr_value_from =
                        qq.pricing_attr_value_from(+)
                 AND NOT EXISTS
                        (SELECT 1
                           FROM qp_modifier_summary_v qms2,
                                qp_secu_list_headers_vl qsl2,
                                qp_pricing_attributes qpall2
                          WHERE     1 = 1
                                AND msi.segment1 = qms2.product_attr_value
