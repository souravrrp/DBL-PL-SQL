...
AS
   ...
   FUNCTION get_category_from_customer_id (p_customer_id NUMBER)
      RETURN VARCHAR2
   AS
          ...
          WHERE     ps.customer_id = p_customer_id
                ...
                AND NOT EXISTS
                           (SELECT 1
                             ...
                             WHERE     ps.cash_receipt_id =
                                          cr.cash_receipt_id
                                   AND ps.org_id = cr.org_id
                                   AND NVL (cr.status, 'AKG') IN ('CCRR',
                                                                  'CC_CHARGEBACK_REV',
                                                                  'NSF',
                                                                  'REV',
                                                                  'STOP'));
   ...
   FUNCTION get_cust_opening_bal_br (p_org_id         NUMBER,
                                     p_customer_id    NUMBER,
                                     p_date_as_on     DATE)
      RETURN NUMBER
   AS
   ...
   BEGIN
      ...
      LOOP
         ...
         THEN
            ...
            FOR x IN (SELECT * FROM ar_trx_errors_gt)
            LOOP
               xx_com_pkg.writelog (
