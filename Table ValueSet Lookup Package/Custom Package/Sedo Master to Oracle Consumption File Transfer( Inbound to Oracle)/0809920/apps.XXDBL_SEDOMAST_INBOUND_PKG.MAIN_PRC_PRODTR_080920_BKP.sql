...
--      show_errors (p_request_id);
   ...
   BEGIN
      ...
      writelog (   'Total number of records for transaction             :'
                || SQL%ROWCOUNT
               );
   ...
   EXCEPTION
      WHEN OTHERS
      THEN
         writelog (   'Error in submitting DBL Data Load Program '
                   || TO_CHAR (SQLCODE)
                   || ': '
                   || SQLERRM
                  );
   ...
   BEGIN
      ...
      LOOP
         ...
         THEN
            BEGIN
               SELECT SUBSTR (r_opm.batch_number,
                              1,
         ...
         THEN
            ...
            END;
         ELSE
            l_validation_error := l_validation_error || ' ITEM_CODE is null.';
         END IF;
...
--                WHERE uom_code = l_lot_uom_code;
   ...
   IS
      ...
      p_validation_level             NUMBER
                                          DEFAULT gme_common_pvt.g_max_errors;
   ...
   BEGIN
      ...
      --
      UPDATE xxdbl_opm_conv_stg
         SET line_api_status = g_default,
             line_api_error_message = NULL
       WHERE 1 = 1         --   request_id = p_request_id --commited on 28-Aug
         AND batch_status <> 0                            --added on 24-JAN-2020 V1.4
      ...
      LOOP
         BEGIN
            ...
            BEGIN
               SELECT MIN (material_detail_id)
                ...
                WHERE gbh.batch_id = gmd.batch_id
                  ...
                  AND gmd.inventory_item_id = r_opm_line.inventory_item_id;
            EXCEPTION
               WHEN OTHERS
               THEN
                  l_api_error :=
                  ...
                  SET line_api_status = g_success,
                      ...
                      line_no =
                         (SELECT gmd.line_no
                           ...
                           WHERE gmd.material_detail_id = l_material_detail_id)
                WHERE ROWID = r_opm_line.h_rowid;
            ...
            ELSE
               ...
               l_material_detail_rec.organization_id := r_opm_line.inv_org_id;
               l_material_detail_rec.line_type := -1;
               --   l_material_detail_rec.line_no := 7;
               l_material_detail_rec.plan_qty := r_opm_line.recipe_qty;  --0;
               l_material_detail_rec.wip_plan_qty := r_opm_line.recipe_qty;
               --0;
               l_material_detail_rec.original_qty := r_opm_line.recipe_qty;
               --0;
               ...
               THEN
                  l_error_msg :=
                     SUBSTR (   'Material Line Creation Error - '
                             || x_message_list1,
                             1,
                             4000
                            );
   ...
   EXCEPTION
      ...
      WHEN OTHERS
      THEN
         writelog (   'Unexpected Error to create batch line - '
                   || TO_CHAR (SQLCODE)
                   || ': '
                   || SQLERRM
                  );
      ...
      LOOP
         ...
         BEGIN
            ...
            BEGIN
               ...
               LOOP
                  ...
                  SELECT moqd.onhand_quantities_id,
                   ...
                   WHERE moqd.organization_id = r_opm_transaction.inv_org_id
            ...
            l_mmti_rec.transaction_date :=
                 TO_DATE (r_opm_transaction.end_time, 'dd/mm/yyyy hh24:mi:ss');
-- end_time is passed as transaction date as per discussion on 31-Aug-19 with DBL --SYSDATE; --r_opm_transaction.end_time;
            ...
            l_mmti_rec.inventory_item_id :=
                                           r_opm_transaction.inventory_item_id;
                                                                       --4912;
            --
            l_mmti_rec.transaction_source_id := r_opm_transaction.batch_id;
            --92039;                   --c1.batch_id;
   ...
   EXCEPTION
      ...
      WHEN OTHERS
      THEN
         writelog (   'Unexpected Error to create transaction - '
                   || TO_CHAR (SQLCODE)
                   || ': '
                   || SQLERRM
                  );
