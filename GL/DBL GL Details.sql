/* Formatted on 1/11/2022 5:32:06 PM (QP5 v5.374) */
SELECT gl.name
           ledger_name,
       hou.name
           operating_unit,
 ...
 WHERE     1 = 1
       ...
       --AND gll.je_header_id = 792510
       AND (   :p_batch_name IS NULL
            OR (UPPER (GLB.name) LIKE UPPER ('%' || :p_batch_name || '%')))
       AND (   :p_journal_name IS NULL
            OR (UPPER (glh.name) LIKE UPPER ('%' || :p_journal_name || '%')))
       AND (   :p_period_name IS NULL
            OR (glh.period_name = UPPER ( :p_period_name)))
       AND ( :p_ou_name IS NULL OR (hou.name = :p_ou_name))
       AND (   :p_ledger_name IS NULL
            OR (UPPER (ou.ledger_name) LIKE
                    UPPER ('%' || :p_ledger_name || '%')));