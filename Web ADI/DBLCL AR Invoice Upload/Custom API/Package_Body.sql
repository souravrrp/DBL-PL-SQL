/* Formatted on 7/12/2020 4:51:07 PM (QP5 v5.287) */
   ...
   IS
      ...
      vl_responsibility_id     NUMBER
         := CASE
               ...
               ELSE '51915'
            END;
      --vl_responsibility_id     NUMBER := NVL (p_responsibility_id, 51915);
      vl_respappl_id           NUMBER
         := CASE
               WHEN p_respappl_id IS NOT NULL THEN p_respappl_id
               ELSE '222'
            END;
      --vl_respappl_id           NUMBER := NVL (p_respappl_id, 222);
      vl_user_id               NUMBER
         := CASE WHEN p_user_id IS NOT NULL  THEN p_user_id ELSE '5958' END;
      --vl_user_id               NUMBER := NVL (p_user_id, 5958);
      vl_org_id                NUMBER
         := CASE WHEN p_org_id IS NULL OR p_org_id='-1' THEN '131'  ELSE p_org_id END;

      --vl_org_id                NUMBER := NVL (p_org_id, 131);
