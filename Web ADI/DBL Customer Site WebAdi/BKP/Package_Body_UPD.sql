/* Formatted on 5/22/2021 10:42:11 AM (QP5 v5.287) */
...
IS
   ...
   BEGIN
      ...
      --------------------------------------------------------
      BEGIN
           SELECT COUNT (*), loc.address1
             INTO l_existing_loc, l_existing_address
             FROM apps.hz_locations loc
            WHERE UPPER (loc.address1) = UPPER (p_address1)
         GROUP BY loc.address1;

         IF ( (l_existing_loc IS NOT NULL) OR (l_existing_loc <> 0))
         THEN
            l_error_message :=
                  l_error_message
               || ','
