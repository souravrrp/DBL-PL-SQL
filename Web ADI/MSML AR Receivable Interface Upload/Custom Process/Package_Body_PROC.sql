...
IS
      ...
      LOOP
         BEGIN
            ...
            VALUES (ra_customer_trx_lines_s.NEXTVAL,    --> interface_line_id,
                    ...
                    ln_cur_stg.CREATED_BY, -- fnd_global.user_id, --> last_updated_by,
                    SYSDATE,                                --> creation_date,
                    ln_cur_stg.CREATED_BY, -- fnd_global.user_id --> created_by,
                    'Y',                                     --> taxable_flag,
   ...
   IS
      --------------------------------------------
      P_SL_NO                 NUMBER
         := TRIM (LPAD (XXDBL.XXDBL_AR_INTERFACE_INV_S.NEXTVAL, 5, '0'));
   ...
   BEGIN
      ...
      */
      ...
      BEGIN
         FOR c
            IN (SELECT NVL (PHA.SEGMENT1, NULL),
                 ...
                 WHERE     pha.type_lookup_code IN ('BLANKET', 'STANDARD')
                       AND NVL (pha.authorization_status, 'INCOMPLETE') =
                              'APPROVED'
         ...
         LOOP
            l_po_price_cnt := l_po_price_cnt + 1;
         END LOOP;
         ...
         THEN
            L_ERROR_MESSAGE :=
                  L_ERROR_MESSAGE
               || ','
               || 'Please check PO price for the item of PO Number: '
               || P_PO_NUMBER;
            L_ERROR_CODE := 'E';
            DBMS_OUTPUT.PUT_LINE (
               'Please check the PO price for PO Number: ' || P_PO_NUMBER);
         ELSE
            DBMS_OUTPUT.PUT_LINE (
               'PO price is validated for PO Number and Item Code.');
         END IF;
      EXCEPTION
         WHEN OTHERS
         THEN
