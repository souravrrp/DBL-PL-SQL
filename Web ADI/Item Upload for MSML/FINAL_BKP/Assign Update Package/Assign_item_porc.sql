/* Formatted on 7/15/2020 12:31:28 PM (QP5 v5.287) */
...
IS
      ...
      LOOP
         CASE
            ...
            THEN
               ...
               --item_catalog_assign (ln_cur_stg.segment1);
               vlp_category_id := 2126;
               assign_item_category (ln_cur_stg.segment1,
   ...
   IS
      ...
      l_init_msg_list           Error_Handler.Error_Tbl_Type;
   ...
   BEGIN
      ...
      SELECT item_catalog_group_id
       ...
       WHERE segment1 = 'Yarn'; -- Item Catalog Category Name, if using Product Hub
       */

      l_item_tbl (1).Transaction_Type := 'CREATE';          --UPDATE  --CREATE
      l_item_tbl (1).Inventory_Item_Status_Code := 'Active';
      l_item_tbl (1).Organization_ID := tm_org_id;
      l_item_tbl (1).Description := tm_Item_description;   -- Item Description
      l_item_tbl (1).Segment1 := tm_item_code;                  -- Item Status
      l_item_tbl (1).Template_Name := tm_template_name;       -- Item template
      --l_item_tbl (1).Item_Catalog_Group_Id := l_item_catalog_group_id; -- Item Catalog Group ID
   ...
   PROCEDURE item_catalog_assign (cp_item_code IN VARCHAR2)
   IS
      l_api_version             NUMBER := 1.0;
      ...
      l_user_name               VARCHAR2 (30) := '100277';
   ...
   BEGIN
      ...
      -- Load the item catalog group id
      SELECT item_catalog_group_id
        INTO p_item_catalog_group_id
        FROM mtl_item_catalog_groups_b
       WHERE segment1 = 'Yarn';

      SELECT inventory_item_id
        INTO cp_inventory_item_id
        FROM mtl_system_items_b
       WHERE segment1 = cp_item_code AND organization_id = 138;
      ...
      l_item_tbl (1).Inventory_Item_Id := cp_inventory_item_id;
      l_item_tbl (1).Organization_Code := 'IMO';
      l_item_tbl (1).Inventory_Item_Status_Code := 'Active';
