/* Formatted on 8/3/2021 9:11:48 AM (QP5 v5.287) */
...
IS
   ...
   FUNCTION upd_customer_add_info (p_cust_id NUMBER)
      RETURN NUMBER
   IS
   ...
   BEGIN
      ...
      LOOP
         ...
         THEN
            COMMIT;
            DBMS_OUTPUT.PUT_LINE (
               'Updation of Customer Account is Successful ');
         ...
         BEGIN
            ...
            THEN
               ...
               CASE
                  ...
                  THEN
                     BEGIN
                        l_return_val := upd_customer_add_info (r.cust_id);
                        COMMIT;
   ...
   BEGIN
      ...
      ------------------------------------------------------
      BEGIN
         IF p_customer_no IS NOT NULL AND p_cust_site_category IS NULL
         THEN
            BEGIN
               SELECT DISTINCT hp.party_id,
                               ca.cust_account_id,
                               ca.account_name,
                               casa.customer_category_code,
                               ca.customer_type
                 INTO l_party_id,
                      l_customer_id,
                      l_customer_name,
      ...
      --------------------------------------------------
      ----------Validate Customer Site Category-------------------
      --------------------------------------------------
      IF p_cust_site_category IS NOT NULL
      THEN
         BEGIN
            SELECT LOOKUP_CODE
