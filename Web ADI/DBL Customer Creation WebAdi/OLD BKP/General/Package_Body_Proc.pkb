/* Formatted on 5/10/2021 10:13:43 AM (QP5 v5.287) */
CREATE OR REPLACE PACKAGE BODY APPS.xxdbl_cust_webadi_pkg
IS
   -- CREATED BY : SOURAV PAUL
   -- CREATION DATE : 10-MAR-2021
   -- LAST UPDATE DATE :10-MAR-2021
   -- PURPOSE : CUSTOMER CREATION WEB ADI
   FUNCTION create_cust_acct_site (p_cust_id NUMBER, p_location_id NUMBER)
      RETURN NUMBER
   IS
      CURSOR c
      IS
   ...
   FUNCTION create_party_site (p_cust_id NUMBER, p_location_id NUMBER)
      RETURN NUMBER
   IS
      CURSOR c
      IS
   ...
   BEGIN
      ...
      LOOP
         ...
         COMMIT;
         DBMS_OUTPUT.put_line ('ln_location_id-' || p_location_id);
         ...
         COMMIT;
         DBMS_OUTPUT.put_line ('ln_party_site_id' || ln_party_site_id);
         l_return_val := create_cust_acct_site (r.cust_id, ln_location_id);
   ...
   FUNCTION create_customer_loc (p_cust_id NUMBER, p_cust_account_id NUMBER)
      RETURN NUMBER
   IS
      CURSOR c
      IS
   ...
   BEGIN
      ...
      LOOP
         ...
         l_return_val := create_party_site (r.cust_id, ln_location_id);
   ...
   BEGIN
      ...
      LOOP
         BEGIN
            ...
            THEN
               l_return_val :=
                  create_customer_loc (i.cust_id, x_cust_account_id);
