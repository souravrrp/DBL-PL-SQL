/* Formatted on 6/28/2021 12:14:08 PM (QP5 v5.287) */
  SELECT *
...
UPDATE xxdbl.xxdbl_cust_creation_tbl cct SET cct.status='Y' WHERE cct.status IS NULL;
...
DELETE xxdbl.xxdbl_cust_creation_tbl cct
 WHERE 
 cct.status IS NULL;
...
SELECT lookup_type,
  ...
  --,FLV.*
  FROM fnd_lookup_values_vl flv
 WHERE 1 = 1 AND UPPER (lookup_type) = 'DBL_CUSTOMER_TYPE' -- UPPER ( :p_lookup_type)
;
...
ORDER BY account_number DESC;
  ...
  SELECT hou.organization_id,
   ...
   WHERE     1 = 1
         ...
         AND SITE_USE_CODE IN ('SHIP_TO', 'BILL_TO','DRAWEE')
         --AND PRIMARY_FLAG='Y'
         --AND STATUS = 'A'
         --AND CUSTOMER_TYPE = 'I'
