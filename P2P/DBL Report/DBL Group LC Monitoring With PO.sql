/* Formatted on 7/19/2020 9:25:09 AM (QP5 v5.287) */
  SELECT led.UNIT_NAME OU_Name,
   ...
   WHERE     1 = 1
         ...
         AND (:P_OPERATING_UNIT IS NULL OR (odd.OPERATING_UNIT = :P_OPERATING_UNIT))
  ...
  SELECT                                    --x.ship_header_id ship_header_id,
    ...
    FROM (SELECT ALLOC.SHIP_HEADER_ID,
           ...
           WHERE     UOM.UOM_CODE = SL.PRIMARY_UOM_CODE
                 ...
                 AND (   :P_OPERATING_UNIT IS NULL OR (pla.org_id = :P_OPERATING_UNIT))
