/* Formatted on 7/27/2021 9:52:16 AM (QP5 v5.287) */
...
AS
   ...
   UNION ALL
     SELECT hou.name OU_Name,
      ...
      WHERE     pha.po_header_id = pla.po_header_id
            ...
            AND pha.po_header_id = rt.po_header_id
            AND pla.po_line_id = rt.po_line_id
            AND isl.ship_line_id = rt.lcm_shipment_line_id(+) -----Updated By Sourav on 13-JUN-21
            AND rt.shipment_header_id = rsh.shipment_header_id
            AND l.attribute14(+) = prod.distribution_id
            AND l.header_id = mtrh.header_id(+)
            AND pha.segment1 = b2b.po_number(+)
   ...
   UNION ALL
     SELECT hou.name OU_Name,
            ...
            ail.invoice_num,
            AIL.TAX_INVOICE_INTERNAL_SEQ ITN,
            ...
            prha.CREATED_BY AS PR_CREATED_BY,
            pha.CREATED_BY AS PO_CREATED_BY,
            ail.CREATED_BY AS INVOICE_CREATED_BY,
            pm.CREATED_BY AS PAYMENT_CREATED_BY,
            rt.CREATED_BY RECEIVED_BY
       FROM apps.po_headers_all pha,
            apps.xxdbl_company_le_mapping_v led,
            apps.ap_suppliers sup,
      ...
      WHERE     pha.po_header_id = pla.po_header_id
            ...
            AND pha.po_header_id = rt.po_header_id
            AND pla.po_line_id = rt.po_line_id
            AND isl.ship_line_id = rt.lcm_shipment_line_id(+) -----Updated By Sourav 13-JUN-21
            AND rt.shipment_header_id = rsh.shipment_header_id
   ...
   SELECT hou.name OU_Name,
          ...
          NULL item_code,
          NULL item_name,
          NULL item_category_1,
          NULL item_category_2,
          NULL item_category_3,
          NULL item_category_4,
          pla.unit_meas_lookup_code uom,
          NULL pr_Quantity,
          NULL PR_price,
          pla.quantity PO_Quantity,
