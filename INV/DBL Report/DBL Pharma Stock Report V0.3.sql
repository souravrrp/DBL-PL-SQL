/* Formatted on 1/8/2022 1:00:53 PM (QP5 v5.374) */
SELECT ledger_name,
       legal_entity_name,
       org_id,
       name,
       ...
       pr_quantity,
       po_quantity,
       ((grn_quantity - quarantine_qty) - ((onhand_quantity - (quarantine_qty + reserve_quantity)+reserve_quantity)))
           grn_quantity,
       quarantine_qty,
  ...
  FROM (  SELECT ou.ledger_name,
                 ...
                 msi.primary_uom_code
                     uom,
                 SUM (prla.quantity)
                     --(CASE WHEN SUM (pll.quantity) = 0 THEN SUM (prla.quantity) ELSE ABS (SUM (prla.quantity) - SUM (pll.quantity)) END)
                     pr_quantity,
                 (CASE
                      ...
                      ELSE
                          ABS (
                              SUM (pll.quantity) - SUM (pll.quantity_received))
                  END)
                     po_quantity,
           ...
           WHERE     1 = 1
                 ...
                 AND (   :p_item_category IS NULL
                      OR (mc.segment1 = :p_item_category))
                 ...
                 AND prha.segment1 IN ('21111003551')
