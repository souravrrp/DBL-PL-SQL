/* Formatted on 7/9/2020 12:01:05 PM (QP5 v5.287) */
CREATE OR REPLACE PACKAGE BODY apps.xxdbl_employee_update
IS
   PROCEDURE update_emp_email_addess
   IS
      ...
      LOOP
         BEGIN
            DECLARE
               ...
               l_per_object_version_number   NUMBER := cur_upd_email.object_version_number;
               l_email_add                   VARCHAR2 (100) := cur_upd_email.EMAIL_ADDRESS;
               l_person_id                   NUMBER := cur_upd_email.person_id;
            BEGIN
               hr_person_api.update_person (
                  p_validate                   => FALSE,
                  p_effective_date             => TO_DATE ('30-JUN-2020', 'DD-MON-YYYY'),
            ...
            UPDATE apps.xxdbl_email_update xur
               SET xur.flag = 'Y'
             WHERE     xur.user_name = cur_upd_email.user_name
