      ...
      THEN
         FOR rec_line IN
            (SELECT lines.customer_number, lines.customer_name,
                    ...
                    lines.item_id line_item_id, line_lot.*
             ...
             SELECT lines.customer_number, lines.customer_name,
              ...
              WHERE 1 = 1
                ...
                AND lines.omshipping_line_status NOT IN
                                       ('CANCELLED') --('CLOSED', 'CANCELLED')
                AND NVL (line_lot.quantity_crt, lines.picking_qty_crt) > 0
                AND lines.omshipping_line_id = line_lot.omshipping_line_id(+))
         LOOP
            BEGIN
               SELECT sales_order_id
                        ...
                        SET reservation_quantity =
                            ...
                            secondary_reservation_quantity =
                               DECODE (secondary_reservation_quantity,
                                       NULL, NULL,
                                         NVL (secondary_reservation_quantity,
                                              0
                                             )
            ...
            SELECT NVL (SUM (xbsl.quantity), 0)
             ...
             WHERE 1 = 1
               ...
               AND SUBSTR (xbsl.order_line_no,
                           1,
                           (INSTR (xbsl.order_line_no, '.', 1, 1) - 1)
                          ) = l_line_number
               AND NOT EXISTS (
                      SELECT 'X'
