/* Formatted on 4/4/2021 2:20:39 PM (QP5 v5.287) */
...
IS
   ...
   PROCEDURE create_resv_yarn (p_omshiping_line_id     IN     NUMBER,
   ...
   FUNCTION available_onhand (p_organization_id     IN NUMBER,
                              p_inventory_item_id   IN NUMBER,
                              p_subinventory_code   IN VARCHAR2 DEFAULT NULL,
                              p_locator_id          IN NUMBER DEFAULT NULL,
                              p_lot_number          IN VARCHAR2 DEFAULT NULL)
   ...
   --
   PROCEDURE split_line (p_transaction_hdr_id   IN     NUMBER,
                         x_message                 OUT VARCHAR2)
   IS
      -- API variables
   ...
   BEGIN
      ...
      FOR rec_line
         IN (SELECT line.*, oola.ordered_quantity, oola.ordered_quantity2
              ...
              WHERE     1 = 1
                    ...
                    AND line.omshipping_line_status NOT IN ('CANCELLED') --('CLOSED', 'CANCELLED')
      ...
      LOOP
         ...
         l_line_tbl (l_line_tbl_index).ordered_quantity :=
            (NVL (rec_line.ordered_quantity, 0) --NVL (rec_line.attribute3, 0)
             - NVL (rec_line.picking_qty_crt, 0));
         l_line_tbl (l_line_tbl_index).ordered_quantity2 :=
            (NVL (rec_line.ordered_quantity2, 0) -- NVL (rec_line.attribute4, 0)
         ...
         oe_order_pub.process_order (
            ...
            x_action_request_tbl       => p_action_request_tbl-- added on 16aug PwC
            );
   ...
   PROCEDURE split_line_new (p_omshipping_header_id   IN     NUMBER,
                             p_omshipping_line_id     IN     NUMBER,
                             x_message                   OUT VARCHAR2)
   IS
      -- API variables
   ...
   BEGIN
      FOR rec_line
         IN (SELECT lines.customer_number,
                    lines.customer_name,
                    lines.customer_id,
                    lines.order_number,
                    lines.order_id,
                    lines.ship_to_warehouse,
                    lines.warehouse_id,
                    lines.order_line_no,
                    ...
                    lines.order_line_id,
                    lines.attribute1 primary_uom,
                    lines.attribute2 sececondary_uom,
                    line_lot.*
               FROM xxdbl_transpoter_headers xxth,
                    xxdbl_transpoter_line xxtl,
              ...
              WHERE     1 = 1
                    ...
                    AND lines.omshipping_line_status NOT IN ('CANCELLED') --('CLOSED', 'CANCELLED')
                    AND lines.omshipping_line_id =
                           line_lot.omshipping_line_id
                    AND NOT EXISTS
                           (SELECT 'X'
         ...
         FOR rec_line
            IN (SELECT lines.customer_number,
                ...
                SELECT lines.customer_number,
                 ...
                 WHERE     1 = 1
                       AND lines.omshipping_line_id = p_omshiping_line_id
                       AND NVL (lines.attribute5, '1') = '1'
                       AND lines.omshipping_line_status NOT IN ('CANCELLED') --('CLOSED', 'CANCELLED')
                       AND NVL (line_lot.quantity_crt, lines.picking_qty_crt) >
                              0
                       AND lines.omshipping_line_id =
                              line_lot.omshipping_line_id(+))
         LOOP
            ...
            ELSE
               ...
               l_rsv.organization_id := rec_line.warehouse_id;
               --mtl_parameters.organization id
               l_rsv.inventory_item_id :=
                  NVL (rec_line.item_id, rec_line.line_item_id);
               --mtl_system_items.Inventory_item_id;
               l_rsv.demand_source_name := NULL;
               l_rsv.demand_source_type_id := 2;
               l_rsv.demand_source_header_id := l_sales_order_id;
               ...
               l_rsv.demand_source_delivery := NULL;
               l_rsv.reservation_uom_code := rec_line.primary_uom;
               l_rsv.reservation_quantity :=
                  NVL (rec_line.quantity_crt, rec_line.picking_qty_crt);
               ...
               IF rec_line.lot_number IS NOT NULL
               THEN
                  l_rsv.lot_number := rec_line.lot_number;     --p_lot_number;
               ELSE
               ...
               IF rec_line.locator_id IS NOT NULL
               THEN
                  l_rsv.locator_id := rec_line.locator_id;
               ELSE
               ...
               IF rec_line.subinventory_code IS NOT NULL
               THEN
                  l_rsv.subinventory_code := rec_line.subinventory_code;
               ELSE
      ...
      THEN
         ...
         LOOP
            ...
            THEN
               ...
               ELSE
                  x_message :=
                     x_message || CHR (10) || 'Unable to find Sales Order ';
               END IF;
            ELSE
               l_rsv.requirement_date := SYSDATE;
         ...
         LOOP
            INSERT INTO mtl_reservations (reservation_id,
                                          ...
                                          task_id,
                                          mcc_code)
                 VALUES (rec_line.reservation_id,
                         rec_line.requirement_date,
                         ...
                         rec_line.primary_uom_code,
                         rec_line.primary_uom_id,
                         ...
                         rec_line.supply_source_name,
                         rec_line.revision,
                         rec_line.subinventory_code,
                         rec_line.subinventory_id,
                         rec_line.locator_id,
                         rec_line.lot_number,
                         rec_line.lot_number_id,
                         rec_line.serial_number,
                         rec_line.serial_number_id,
                         rec_line.partial_quantities_allowed,
                         rec_line.auto_detailed,
                         rec_line.pick_slip_number,
                         rec_line.lpn_id,
                         ...
                         rec_line.program_id,
                         rec_line.program_update_date,
                         rec_line.attribute_category,
                         rec_line.attribute1,
                         rec_line.attribute2,
                         rec_line.attribute3,
                         rec_line.attribute4,
                         ...
                         rec_line.ship_ready_flag,
                         rec_line.n_column1,
                         rec_line.detailed_quantity,
                         rec_line.cost_group_id,
                         rec_line.container_lpn_id,
                         rec_line.staged_flag,
                         rec_line.secondary_detailed_quantity,
                         rec_line.secondary_reservation_quantity,
                         rec_line.secondary_uom_code,
                         rec_line.secondary_uom_id,
                         rec_line.crossdock_flag,
                         ...
                         rec_line.demand_ship_date,
                         rec_line.exception_code,
   ...
   BEGIN
      FOR rec_line
         IN (SELECT lines.customer_number,
                    ...
                    lines.attribute2 sececondary_uom,
                    lines.item_id,
                    lines.omshipping_header_id,
                    lines.omshipping_line_id,
                    lines.item_code,
                    lines.ROWID xxrowid
               FROM xxdbl_omshipping_line lines
              WHERE     1 = 1
      ...
      LOOP
         ...
         BEGIN
                     ...
                     WHERE oola1.ship_set_id =
                              (SELECT oola.ship_set_id
                                ...
                                WHERE     oola.header_id = rec_line.order_id
                                      AND oola.line_id =
                                             rec_line.order_line_id)) xx
             WHERE     1 = 1
                   AND ordered_quantity > xxdbl_shiping_tran_crp_pkg.available_to_reserve (
            ...
            INSERT INTO xxdbl_order_ship_set (header_id, line_id)
            ...
            THEN
               ...
               DELETE xxdbl_omshipping_line
                WHERE     omshipping_header_id = p_omshipping_header_id
                      AND omshipping_line_id = p_omshipping_line_id--ROWID = rec_line.xxrowid
               ;
         ...
         ELSE
            ...
            ELSE
               x_message :=
                  x_message || CHR (10) || 'Ship Set Grouping Error: ';
            END IF;
         END IF;
      END LOOP;
   ...
   PROCEDURE split_line_new1 (p_omshipping_hdr_id   IN     NUMBER,
                              x_message                OUT VARCHAR2)
   IS
      -- API variables
      l_header_rec                   oe_order_pub.header_rec_type;
   ...
   BEGIN
      ...
      FOR rec_line
         IN (SELECT line.*,
                    ...
                    oola.CONTEXT,
                    oola.attribute1 oola_attribute1,
   ...
   PROCEDURE transact_move_order (p_move_order_line_id   IN     NUMBER,
                                  p_api                  IN     VARCHAR2,
   ...
   IS
      ...
      CURSOR c_mo_details
      IS
         SELECT mtrh.header_id h_1,
                mtrh.request_number,
                mtrh.move_order_type,
                mtrh.organization_id o_1,
                mtrl.*,
   ...
   BEGIN
      ...
      LOOP
         ...
         inv_pick_wave_pick_confirm_pub.pick_confirm (
            ...
            p_transaction_date     => p_transaction_date--a_l_transaction_date
                                                        -- l_transaction_date
            );
         ...
         THEN
            ...
            THEN
               ...
               LOOP
                  ...
                  ELSE
                     p_return_message :=
                        p_return_message || CHR (10) || a_x_msg_data;
                  END IF;
               END LOOP;
   ...
   BEGIN
      ...
      LOOP
         ...
         /*------Comment on: 04-Apr-21 
         --------Purpose: Invoice Date based on Ship Confirm Date
         --------Updated By: Sourav Paul
         --------Update Date: 26-Apr-21 
         BEGIN
         ...
         */
         ...
         THEN
            ...
            LOOP
               ...
               ELSE
                  p_return_message :=
                     p_return_message || CHR (10) || l_msg_data;
               END IF;
            END LOOP;
   ...
   PROCEDURE pick_release_pick_rule (p_delivery_id      IN     NUMBER,
                                     p_return_status       OUT VARCHAR2,
                                     p_return_message      OUT VARCHAR2)
   IS
   ...
   PROCEDURE pick_ship_process (p_transaction_hdr_id   IN     NUMBER,
                                x_message                 OUT VARCHAR2)
   IS
      ...
      l_ship_confirm_rule             VARCHAR2 (240) := 'SCBL Bulk Ship Confirm';
      l_line_tbl                      oe_order_pub.line_tbl_type;
      l_header_rec                    oe_order_pub.header_rec_type;
      ...
      l_move_order_line_id            mtl_txn_request_lines.line_id%TYPE := NULL;
      l_move_order_quantity           NUMBER := NULL;
      l_inv_period_status             org_acct_periods.open_flag%TYPE := NULL;
      l_acct_period_id                org_acct_periods.acct_period_id%TYPE := NULL;
      l_rowid                         ROWID := NULL;
   ...
   BEGIN
      FOR rec_headers
         IN (  SELECT line.ship_to_warehouse,
                      line.warehouse_id,
                      line.delivery_challan_number,
      ...
      LOOP
         FOR rec_lines
            IN (SELECT lines.omshipping_header_id,
                       lines.omshipping_line_id,
                       lines.org_id,
                       lines.customer_number,
                       lines.customer_name,
                       lines.customer_id,
                       lines.order_number,
                       lines.order_id,
                       ...
                       lines.item_id,
                       lines.grade,
                       lines.primary_quantity,
                       lines.secondary_quantity,
                       lines.primary_onhand_quantity,
                       lines.secondary_onhand_quantity,
                       lines.schedule_ship_date,
                       lines.shipment_priority_code,
                       lines.shipment_priority_meaning,
                       lines.picking_qty_crt,
                       lines.picking_qty_sft,
                       lines.omshipping_line_status,
                       lines.delivery_challan_number,
                       lines.transport_challan_number,
                       lines.transport_name,
                       lines.transport_id,
                       lines.delivery_id,
                       lines.delivery_detail_id,
                       lines.delivery_date,
                       lines.attribute1,
                       lines.attribute2,
                       lines.attribute3,
            ...
            BEGIN
               ...
               SELECT xx.delivery_detail_id,
                 ...
                 INTO l_delivery_detail_id,
                      ...
                      l_date_shipped,
                      l_released_status
                 FROM (  SELECT NVL (MAX (NVL (delivery_detail_id, -1)), -1)
                                   delivery_detail_id,
                                NVL (MAX (NVL (delivery_id, -1)), -1)
                                   delivery_id,
                                MAX (latest_pickup_date) latest_pickup_date
                           /*date_shipped,*/
                           FROM wsh_deliverables_v /*wsh_dlvy_deliverables_v*/
                                                  wdlsv
                          WHERE     1 = 1
                ...
                WHERE     1 = 1
                      AND xx.delivery_detail_id = wdlsv2.delivery_detail_id/*AND (   (xx.delivery_id > 0)
                  ...
                  BEGIN
                     SELECT xx.delivery_detail_id,
                       ...
                       INTO l_delivery_detail_id,
                            ...
                            l_date_shipped,
                            l_released_status
                       FROM (  SELECT NVL (MAX (NVL (delivery_detail_id, -1)),
                                           -1)
                                         delivery_detail_id,
                                      NVL (MAX (NVL (delivery_id, -1)), -1)
                                         delivery_id,
                                      MAX (latest_pickup_date)
                                         latest_pickup_date
                                 /*date_shipped,*/
                                 FROM wsh_deliverables_v /*wsh_dlvy_deliverables_v*/
                                                        wdlsv
                            ...
                            wsh_deliverables_v wdlsv2
                      WHERE     1 = 1
                            AND xx.delivery_detail_id =
                                   wdlsv2.delivery_detail_id
                     ...
                     THEN
                        BEGIN
                           ...
                           BEGIN
                              ...
                              FOR rec_move_order_line
                                 IN (SELECT mtrh.header_id,
                                            mtrh.request_number,
                                            mtrh.move_order_type,
                              ...
                              LOOP
                                 ...
                                 FOR rec_lots_headers
                                    IN (  SELECT lots.subinventory_code,
                                                 lots.locator_id,
                                                 NVL (
                                                    SUM (
                                                       NVL (lots.quantity_crt,
                                                            0)),
                                                    0)
                                                    quantity_crt,
                                                 NVL (
                                                    SUM (
                                                       NVL (lots.quantity_sft,
                                                            0)),
                                                    0)
                                                    quantity_sft
                                            FROM xxdbl_omshipping_lot_lines
                                                 lots
                                       ...
                                       INSERT
                                         INTO mtl_material_transactions_temp (
                                          ...
                                          VALUES (
                                                    ...
                                                    -- ITEM_DESCRIPTION
                                                    rec_lots.attribute1-- reservation_id
                                                    );
   ...
   FUNCTION get_sales_channel (p_header_id        IN NUMBER,
   ...
   FUNCTION return_available_quantity (p_inevntory_item_id   IN NUMBER,
   ...
   BEGIN
      ...
      l_final_return :=
         'Item Code ' || p_item_code /*|| CHR (10)
                                     || 'Primary Available Quantity is: '
                                     || l_qty_pri*/
         || CHR (10) || 'Secondary Available Quantity is: ' || l_qty_sec;
      RETURN l_final_return;
   END return_available_quantity;

   FUNCTION return_avail_quantity_new (p_inevntory_item_id   IN NUMBER,
                                       p_ship_from_org_id    IN NUMBER,
                                       p_item_code           IN VARCHAR2,
                                       p_pri_uom             IN VARCHAR2,
                                       p_sec_uom             IN VARCHAR2,
   ...
   BEGIN
      BEGIN
         SELECT NVL (SUM (ordered_quantity), 0),
           ...
           INTO l_ordered_quantity, l_ordered_quantity2
           FROM oe_order_lines_all
          WHERE     1 = 1
   ...
   FUNCTION get_split_line_exists (p_order_header_id   IN NUMBER,
   ...
   BEGIN
      ...
      THEN
         BEGIN
              SELECT line_number,
                     NVL (
                        ...
                        0)
                INTO l_line_number, l_order_original_qty
                FROM oe_order_lines_all
         ...
         BEGIN
            SELECT NVL (SUM (xbsl.quantity), 0)
              INTO l_total_bs_qty
             ...
             WHERE     1 = 1
                   ...
                   AND NOT EXISTS
                          (SELECT 'X'
                             FROM xxdbl_bill_stat_headers xbsh
                            WHERE     1 = 1
                                  AND xbsl.bill_stat_header_id =
                                         xbsh.bill_stat_header_id
                                  AND xbsh.bill_stat_status = 'CANCELLED')/*SELECT NVL (SUM (quantity), 0)
                                                                            INTO l_total_bs_qty
