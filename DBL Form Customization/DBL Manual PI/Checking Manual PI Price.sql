/* Formatted on 2/3/2021 10:23:06 AM (QP5 v5.354) */
SELECT mcv.concatenated_segments, llv.operand, llv.*
 ...
 WHERE     lhv.list_header_id = llv.list_header_id
       ...
       AND (   :p_article_ticket IS NULL
            OR (UPPER (mcv.concatenated_segments) LIKE
                    UPPER ('%' || :p_article_ticket || '%')))
...
SELECT cat.segment1 || '.' || cat.segment2     article,
 ...
 WHERE     1 = 1
       ...
       AND cat.segment1 || '.' || cat.segment2 = 'ES01150.COLOUR';


--------------------------------------------------------------------------------

SELECT operand     list_price
