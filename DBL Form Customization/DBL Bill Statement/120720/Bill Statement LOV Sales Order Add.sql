SELECT   order_number, order_id header_id
    FROM (SELECT    e.line_number
                 || DECODE (e.shipment_number,
                            NULL, NULL,
                            '.' || e.shipment_number
                           )
                 || DECODE (e.option_number,
                            NULL, NULL,
                            '.' || e.option_number
                           ) order_line_no,
                 ...
                 * e.unit_list_price VALUE,
                 d.header_id order_id, e.line_id order_line_id,
                 e.inventory_item_id, d.packing_instructions style_number,
           ...
           WHERE a.category_set_name = 'DBL_SALES_CAT_SET'
             ...
             AND xxdbl_shiping_tran_crp_pkg.get_split_line_exists
                                                                 (e.header_id,
                                                                  e.line_id
                                                                 ) = 0
          UNION ALL
