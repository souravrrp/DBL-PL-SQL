DECLARE
   ...
   l_fst_aprv_by_person          NUMBER;
   l_snd_aprv_by_person          NUMBER;
...
BEGIN
   ...
   SELECT fu.user_name, fu.employee_id
     INTO v_emp_no, l_employee_id
     FROM fnd_user fu
    WHERE fu.user_id = p_user_id;

   SELECT first_approver,fst_aprv_by_person
     INTO l_first_approver,l_fst_aprv_by_person
     FROM xxdbl.xxdbl_iou_req_dtl
    WHERE iou_req_id = l_iou_req_id;
   
   SELECT second_approver,snd_aprv_by_person
     INTO l_second_approver,l_snd_aprv_by_person
     FROM xxdbl.xxdbl_iou_req_dtl
    WHERE iou_req_id = l_iou_req_id;
   ...
   IF     :XXDBL_IOU_REQ_DTL.IOU_REQ_ID IS NOT NULL
      ...
      AND l_snd_aprv_by_person IS NOT NULL
      AND l_fst_aprv_by_person=l_employee_id
   THEN
   ...
   ELSIF     :XXDBL_IOU_REQ_DTL.IOU_REQ_ID IS NOT NULL
         AND :XXDBL_IOU_REQ_DTL.PRESENT_STATUS = 'CREATED'
         AND l_snd_aprv_by_person IS NULL
         AND l_fst_aprv_by_person=l_employee_id
   THEN
   ...
   ELSIF     :XXDBL_IOU_REQ_DTL.IOU_REQ_ID IS NOT NULL
         ...
         AND l_snd_aprv_by_person=l_employee_id
   THEN
      :XXDBL_IOU_REQ_DTL.FINAL_APPROVER := v_emp_no;
      SET_ALERT_PROPERTY ('alt_approve',
                          alert_message_text,
