/* Formatted on 2/13/2021 12:07:39 PM (QP5 v5.354) */
SELECT                                                                  --sh.*
       xsl.*
  FROM xxdbl.xxdbl_sqrpol_headers    xsh,
       xxdbl.xxdbl_sqrpol_operation  xso,
       xxdbl.xxdbl_sqrpol_line       xsl
 WHERE     1 = 1
       AND xsh.header_id = xso.header_id(+)
       AND xsh.header_id = xsl.header_id(+)
       AND xso.shift_id = xsl.shift_id(+)
       AND (   :p_organization_code IS NULL OR (xsh.organization_code = :p_organization_code))
       AND (   :p_sft_rcd_counter IS NULL OR (xso.shift_counter = :p_sft_rcd_counter))
       AND (   :p_stock_locator IS NULL OR (xsl.stock_locator = :p_stock_locator))
       --AND xso.shift_counter IN ('3040 - C - 002')
       --AND xsl.stock_locator = 'WPallet-185'
       --AND xsl.line_no = 'Phase2'
       --AND xsh.batch_no = '3040'
       --AND NOT EXISTS(SELECT 1 FROM inv.mtl_material_transactions  mmt WHERE mmt.transaction_source_id = xsl.batch_id AND mmt.locator_id = xsl.stock_locator_id)
       AND ( :p_item_code IS NULL OR (xsl.raw_item_code = :p_item_code))
       AND ( :p_batch_no IS NULL OR (xsh.batch_no = :p_batch_no));
       
       

--------------------------------------------------------------------------------

  SELECT grb.routing_no                        AS process,
   ...
   WHERE     1 = 1
         ...
         AND grb.owner_organization_id = gbh.organization_id
         ...
         --AND NOT EXISTS(SELECT 1 FROM xxdbl.xxdbl_sqrpol_line xsl WHERE gbh.batch_id=xsl.batch_id AND mmt.locator_id = xsl.stock_locator_id)
