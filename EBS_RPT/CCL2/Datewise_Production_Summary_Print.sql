WITH TmpDate
     AS (SELECT TO_DATE (:p_StartDate, 'DD/MM/RRRR hh12:mi:ssAM') + ROWNUM - 1 D_DATE
         FROM DUAL
         CONNECT BY LEVEL <= TO_DATE (:p_EndDate, 'DD/MM/RRRR hh12:mi:ssAM')
                       - TO_DATE (:p_StartDate, 'DD/MM/RRRR hh12:mi:ssAM') + 1),
     ...
     AS (  SELECT TRUNC (gbs.ACTUAL_CMPLT_DATE) AS ACTUAL_CMPLT_DATE,
             ...
             FROM gme.GME_BATCH_HEADER H,
                  ...
                  ---------------------------
                  GMD_OPERATIONS OPN,
                  (SELECT actual_qty, batch_id FROM gme_material_details WHERE line_type = -1 AND DTL_UM = 'KG') Tq,
