...
from
apps.fnd_user fnd, apps.icx_sessions icx, apps.fnd_responsibility_tl frt
where
...
SELECT p.employee_number "Employee Number", p.full_name "Employee Name", 
p.attribute2 "Other Name", p.start_date "Hire Date", 
p.email_address "Email in HR Window", o.NAME "Organization Name", 
u.user_name "User Name", u.email_address "Email in User Window" 
...
SELECT A.menu_id , B.sub_menu_id , C.prompt , B.function_id, D.user_function_name, B.entry_sequence FROM 
apps.Fnd_menus A, 
...
B.function_id = D.function_id AND 
C.LANGUAGE='US' AND 
D.LANGUAGE='US' AND 
A.menu_id=68077; 
