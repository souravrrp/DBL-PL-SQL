--Transaction Types in Receivables R12
SELECT rctta.set_of_books_id      ledger_id,
       rctta.org_id               operating_unit,
       rctta.cust_trx_type_id     trx_type_id,
       rctta.name                 trx_type_name,
       rctta.description          trx_type_description,
       rctta.TYPE                 trx_type,
       rctta.status               trx_status
       --,rctta.*
  FROM apps.ra_cust_trx_types_all rctta
 WHERE     1 = 1
       AND rctta.org_id = NVL ( :P_ORG_ID, rctta.org_id)
       AND (   ( :p_trx_type IS NULL) OR (UPPER (rctta.name) LIKE UPPER ('%' || :p_trx_type || '%')));

--Transaction Types Details in Receivables R12
SELECT hou.name                               operating_unit,
       xep.name                               legal_entity,
       rctt.cust_trx_type_id                  rcv_trx_type_id,
       rctt.name,
       rctt.description,
       al.meaning                             class,
